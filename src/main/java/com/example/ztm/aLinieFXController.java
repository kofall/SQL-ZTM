        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                ...
                if(selectedItems.isEmpty()) {
                    ...
                    alert.setHeaderText(null);
                    alert.setContentText("Linia nie została wybrana!");
                    alert.showAndWait();
                }else {
                    Map<String,Object> record = selectedItems.get(0);
                    Swapper swapper = new Swapper(true, null, user, null, null, "admin/insertUpdateLinieFXML", "Linia");
                    ((aInsertUpdateLinieFXController) swapper.getController()).myInitialize(this, record);
                }
            } catch (IOException e) {
                e.printStackTrace();
                return;
        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                ...
                if(selectedItems.isEmpty()) {
                    ...
                    ((sSureFXController) swapper.getController()).myInitialize(this, record, "nr", "nr_lini", "linia");
                }
            } catch (IOException e) {
                e.printStackTrace();
                return;
