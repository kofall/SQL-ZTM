...
public class aPojazdyFXController implements Initializable {
    ...
    private void add(MouseEvent event) {
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                new Swapper(true, null, user, null, null, "admin/insertUpdatePojazdyFXML", "Pojazd");
            } catch (IOException e) {
                e.printStackTrace();
                return;
            }
    ...
    private void modify(MouseEvent event) {
        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                ...
                ((aInsertUpdatePojazdyFXController) swapper.getController()).myInitialize(null/*RECORD*/);
            } catch (IOException e) {
                e.printStackTrace();
                return;
            }
