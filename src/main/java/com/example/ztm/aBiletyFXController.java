 ...
 * @author kofal
 */
public class aBiletyFXController implements Initializable {
    ...
    @FXML
    private TableColumn<Map, String> tc_Czas_obowiazywania;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tc_Czas_obowiazywania.setCellValueFactory(new MapValueFactory<>("czas_obow"));
    ...
    private void find(MouseEvent event) {
        if(event.getButton() == MouseButton.PRIMARY) {
            ...
            try {
                conn = DriverManager.getConnection(connectionString,
                        connectionProps);
                try (PreparedStatement pstmt1 = conn.prepareStatement("SELECT * FROM rodzaje_biletow WHERE nazwa LIKE '%'||?||'%'"); ){
    ...
    private void add(MouseEvent event) {
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                Swapper swapper = new Swapper(true, null, user, null, null, "admin/insertUpdateBiletyFXML", "Bilet");
                ((aInsertUpdateBiletyFXController) swapper.getController()).myInitialize(this, null);
            } catch (IOException e) {
                e.printStackTrace();
                return;
            }
        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                ...
                if(selectedItems.isEmpty()) {
                    ...
                    alert.setHeaderText(null);
                    alert.setContentText("Rodzaj nie został wybrany!");
                    alert.showAndWait();
                }else{
                    Map<String,Object> record = selectedItems.get(0);
                    Swapper swapper = new Swapper(true, null, user, null, null, "admin/insertUpdateBiletyFXML", "Bilet");
                    ((aInsertUpdateBiletyFXController) swapper.getController()).myInitialize(this, record);
                }
        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            try {
                ...
                if(selectedItems.isEmpty()) {
                    ...
                    ((sSureFXController) swapper.getController()).myInitialize(this, record, "nazwa", "nazwa", "rodzaje_biletow");
                }
